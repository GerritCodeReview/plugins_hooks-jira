...
public class JiraPluginConfig {
  ...
  public JiraPluginConfig(@GerritServerConfig Config gerritConfig,
    ...
    this.rpcUrl = getString("rpcUrl");
    this.gitBasePath =
        new File(gerritSite, getStringDefault("gerrit", "basePath", "git"));
    this.issueRegex =
        getStringDefault("commentLink", "jira", "match",
            "(\\[[a-zA-Z0-9]*-[0-9]*\\])");
    
    gitwebUrl = getGitwebUrl(gerritConfig);
  }
  ...
  private String getGitwebUrl(Config gerritConfig) throws MalformedURLException {
    ...
    if(webUrl == null) {
      ...
      if(gerritConfig.getString("gitweb", null, "cgi") != null) {
        String canonicalUrl = gerritConfig.getString("gerrit", null, "canonicalWebUrl");
  ...
  private String getStringDefault(String section, String key,
      String defaultValue) {
    return Objects.firstNonNull(config.getString(section, null, key),
        defaultValue);
  }

  private String getStringDefault(String section, String subsection,
      String key, String defaultValue) {
    return Objects.firstNonNull(config.getString(section, subsection, key),
        defaultValue);
  }

  private String getString(String name) throws InvalidConfigurationException {
