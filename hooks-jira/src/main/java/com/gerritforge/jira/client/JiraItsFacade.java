...
import com.gerritforge.its.InvalidTransitionException;
import com.gerritforge.its.ItsFacade;
import com.gerritforge.its.ItsIssue;
import com.gerritforge.its.ItsFacade.Check;
...
public class JiraItsFacade implements ItsFacade {
  ...
  @Override
  public ItsIssue getIssue(final String issueKey) throws IOException {
    return execute(new Callable<ItsIssue>(){
      ...
      public ItsIssue call() throws Exception {
        ...
        ItsIssue result = (issue == null ? null : new ItsIssue(issueKey, issue));
        log.debug("Got issue " + issueKey + ", " + result);
        return result;
      }});
  }
  ...
  private String healthCheckAccess() throws RemoteException {
    ...
    JiraToken token = client.login(getUsername(), getPassword());
    client.logout(token);
